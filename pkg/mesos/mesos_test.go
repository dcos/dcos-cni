...
var _ = Describe("Mesos", func() {
	Describe("Testing container IP", func() {
		...
		Context("With `MESOS_CONTAINER_IP` set to IPv6", func() {
			It("Testing `MESOS_CONTAINER_IP` with IPv6", func() {
				os.Setenv("MESOS_CONTAINER_IP", "1::1")
				ip, err := mesos.ContainerIP()
				Expect(err).To(HaveOccurred(), "We should not allow IPv6 addresses to be set: %s", ip)
			})
		})
